...
try{
 ...
 assert.equal(await page.getByRole('textbox',{name:'프로젝트 이름',exact:true}).inputValue(),'기존 제목');await page.getByRole('button',{name:'분석 결과 검토',exact:true}).click();await page.getByLabel('광고에 사용할 상품·옵션').selectOption('1');await page.getByLabel('광고에 사용할 상품·옵션').selectOption('0');
