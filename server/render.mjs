...
export async function duration(fid){let n;if(process.env.FFPROBE_PATH){n=Number(await run(process.env.FFPROBE_PATH,['-v','error','-show_entries','format=duration','-of','csv=p=0',filePath(fid)]));}else{const output=await run(process.env.FFMPEG_PATH||'ffmpeg',['-v','error','-i',filePath(fid),'-f','null','-','-progress','pipe:1']);n=Math.max(...[...String(output).matchAll(/out_time_us=(\d+)/g)].map(m=>Number(m[1])/1e6));}assert(Number.isFinite(n)&&n>0,'미디어 길이를 확인하지 못했습니다.');return n;}
...
export async function merge(p){
 ...
 try{
  ...
  const declared=p.scenes.reduce((n,s)=>n+Number(s.seconds||0),0);
  assert(declared>0,'씬 길이를 확인해 주세요.');
  const scale=target/declared;
  let allocated=0;
  const sceneDurations=p.scenes.map((s,i)=>{const seconds=i===p.scenes.length-1?Number((target-allocated).toFixed(3)):Number((Number(s.seconds)*scale).toFixed(3));allocated+=seconds;return seconds;});
  let start=0,events=[];
  for(let i=0;i<p.scenes.length;i++){
   ...
   const filter='[0:v]scale=1080:1920:force_original_aspect_ratio=increase,crop=1080:1920,setsar=1,fps=30,tpad=stop_mode=clone:stop_duration=30[v]';
   args.push('-filter_complex',filter,'-map','[v]','-map','1:a:0','-af','apad','-t',String(seconds),'-c:v','libx264','-preset','fast','-pix_fmt','yuv420p','-c:a','aac','-ar','48000','-ac','2',`scene-${i}.mp4`);
  ...
  args.push('-t',String(target),'-c:v','libx264','-preset','fast','-c:a','aac','-movflags','+faststart','final.mp4');
  await run(process.env.FFMPEG_PATH||'ffmpeg',args,tmp);
